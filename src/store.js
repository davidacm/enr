...
Vue.use(Vuex);
import { local, getItem } from './utils';
...
const store = new Vuex.Store({
  state: {
    textFile: getItem('textFile', ''),
    numAnnotations: getItem('numAnnotations', true),
    useSlider: getItem('useSlider', false),
  ...
  mutations: {
    realtimePersistentFields (state, {field, value}) {
      state[field] = value;
      local.setItem(field, value);
    },
    commonFields (state, {field, value}) {
      state[field] = value;
    },
...
export const computedHelper = function (fields, handler, enableWriting = true) {
  ...
  for (let k of fields) {
    ...
    if (enableWriting) {
      obj[k].set = v => {
        store.commit(handler, {field:k, value:v});
      }
    }
  }
  return obj;
